...
//-- Control stuff
typedef micron::DiscreteSystem<3,1,1> Controller;
//-- Gamma - 2.7
//--  Weight1 - 0.35
//-- Weight2 - 1.0
//-- jacl
Controller::state_matrix_t Ak = {
   0.998432, 0.000099, 0.000192,
   -0.108719, 0.988804, 3.243435,
   -0.030702, -0.001133, 0.331035
};
Controller::input_matrix_t Bk = {
  -0.003858, -0.028090, 0.000155 
};
Controller::output_matrix_t Ck = {
  -1.156189, -0.007224, -0.035401
};
...
//  vel = ss_counter;
  vel = (double)ss_counter / (NUM_LATTICE * SAMPLING_TIME * 1e-6);
  ss_counter = 0;
...
void speedSensorISR(){
  ...
  if(micros()-debounce > 500 && digitalRead(SPEED_SENSOR_PIN)){
    debounce = micros();
    ++ss_counter; 
  }  
}
...
void loop() {  
  ...
  if(diff_time >= SAMPLING_TIME){
    ...
    ref = map(analog_read, 0, 1024, -179, 180);
